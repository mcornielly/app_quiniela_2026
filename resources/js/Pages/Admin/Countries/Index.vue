...
import CountryForm from '@/Components/Admin/FormDrawer/CountryForm.vue'
...
const deleteSelected = async () => {
    ...
    if(selectedItems.value.length === 0){
        notifyError("No countries selected")
        return
    }

    try{

        await confirmDelete(`Delete ${selectedItems.value.length} countries?`)

        router.delete(route('admin.countries.bulkDelete'), {
            data: {
                ids: selectedItems.value
            },
            preserveScroll: true,
            onSuccess: () => {
                notifySuccess("Countries deleted")
                selectedItems.value = []
                router.reload()
            }
        })
...
const confirmDeleteItem = (item) => {
    ...
    router.delete(route('admin.countries.destroy', item.id), {
        preserveScroll: true,
        onSuccess: () => {
                ...
                <div class="overflow-x-auto">
                    <div class="inline-block min-w-full align-middle">
                        ...
                        <div class="overflow-hidden shadow">
                            ...
                            <div v-if="countries.data.length === 0" class="py-12 text-center text-gray-500 dark:text-gray-400">
                                No countries found.
                            </div>
