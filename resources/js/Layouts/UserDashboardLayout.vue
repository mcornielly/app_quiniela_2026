...
const applyTheme = (theme) => {
    ...
    document.documentElement.classList.toggle('dark', resolveActiveTheme(theme) === 'dark')
}
...
const toggleTheme = () => {
    applyTheme(resolveActiveTheme(currentTheme.value) === 'dark' ? 'light' : 'dark')
}
    ...
    <div class="min-h-screen bg-slate-100 text-slate-900 transition-colors dark:bg-slate-950 dark:text-slate-100">
        ...
        <header class="sticky top-0 z-40 border-b border-white/80 bg-white/88 backdrop-blur-xl dark:border-slate-800 dark:bg-slate-950/92">
            <div class="mx-auto flex max-w-7xl items-center justify-between gap-6 px-4 py-3 sm:px-6 lg:px-8">
                ...
                <div class="flex items-center gap-3">
                    <button
                        type="button"
                        @click="toggleTheme"
                        class="hidden rounded-full border border-slate-200/80 bg-white p-3 text-slate-600 shadow-sm transition hover:border-primary-300 hover:text-slate-950 sm:inline-flex dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300 dark:hover:border-primary-500 dark:hover:text-white"
                        :aria-label="resolveActiveTheme(currentTheme) === 'dark' ? 'Activar modo claro' : 'Activar modo oscuro'"
                    >
                        <SunIcon v-if="resolveActiveTheme(currentTheme) === 'dark'" class="h-4 w-4" />
                    ...
                    <button
                        ...
                        :class="currentTheme === 'system' ? 'text-primary-600 dark:text-primary-300' : 'text-slate-500 dark:text-slate-400'"
                        class="hidden text-[11px] font-semibold uppercase tracking-[0.22em] transition hover:text-slate-950 lg:inline-flex dark:hover:text-white"
                    >
                        Auto
                    </button>
                    ...
                    <Dropdown align="right" width="48" content-classes="py-2 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xl">
                        <template #trigger>
                            <button type="button" class="inline-flex items-center gap-3 rounded-2xl bg-transparent px-1 py-1 text-left transition hover:opacity-90">
                                <div class="flex h-10 w-10 items-center justify-center rounded-full bg-primary-100 font-semibold text-primary-700 dark:bg-primary-500/15 dark:text-primary-300">
            ...
            <div
                ...
                class="border-t border-slate-200/80 bg-slate-950 text-white dark:border-slate-800 dark:bg-slate-900"
            >
            ...
            <div v-if="mobileNavOpen" class="border-t border-slate-200/80 bg-white px-4 py-4 lg:hidden dark:border-slate-800 dark:bg-slate-950">
                ...
                <div class="mt-4 flex items-center gap-3">
                    <button
                        type="button"
                        @click="toggleTheme"
                        class="inline-flex rounded-full border border-slate-200/80 bg-slate-50 p-3 text-slate-600 transition dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300"
                        :aria-label="resolveActiveTheme(currentTheme) === 'dark' ? 'Activar modo claro' : 'Activar modo oscuro'"
                    >
                        <SunIcon v-if="resolveActiveTheme(currentTheme) === 'dark'" class="h-4 w-4" />
                        <MoonIcon v-else class="h-4 w-4" />
                    ...
                    <button
                        type="button"
                        @click="setTheme('system')"
                        class="rounded-full border border-slate-200/80 bg-slate-50 px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-500 transition dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400"
                    >
                        Auto
                    </button>
                </div>
